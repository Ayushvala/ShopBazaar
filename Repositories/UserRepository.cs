...
namespace Onlinecom.Repositories
{
    public class UserRepository : IUserRepository
    {
        ...
        public List<ProductModel> GetAllproducts()
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            using (SqlCommand cmd = new SqlCommand("SELECT * FROM Product2", conn))
            {
                conn.Open();
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        product.Add(new ProductModel
                        {
                            ...
                            Oprice = (decimal)reader["Oprice"],
                            Dprice = (decimal)reader["Dprice"],
        ...
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            using (SqlCommand cmd = new SqlCommand("DELETE FROM Product2 WHERE Id = @Id", conn))
            {
        ...
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            using (SqlCommand command = new SqlCommand("INSERT INTO Cart1 (UserId, ProductId) VALUES (@UserId, @ProductId)", connection))
            {
                command.Parameters.AddWithValue("@UserId", userId);
        ...
        public List<ProductModel> GetAllproduct()
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            using (SqlCommand cmd = new SqlCommand("SELECT * FROM Product2", conn)) // Ensure "Product" is your table name  
            {
                conn.Open();
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
